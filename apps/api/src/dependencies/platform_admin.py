...
class ScrapingAdminContext:
    ...
    Scraping controls operate on the shared legal-harvesting pipeline and are
...
async def get_scraping_admin_operator_context(
    ...
    """Require platform admin access for scraping operator routes."""
    ctx = await get_platform_admin_operator_context(auth, jwt_payload, db)
    return ScrapingAdminContext(user=ctx.user, tenant=ctx.tenant, workspace_role=None)


def require_scraping_admin_operator():
    """Scraping operator dependency for platform admins."""
