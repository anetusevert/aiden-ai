...
import { apiClient, type WikiGraphNode, type WikiGraphEdge } from '@/lib/apiClient';
...
export function WikiGraph() {
  ...
  const initGraph = useCallback((nodes: WikiGraphNode[], edges: WikiGraphEdge[]) => {
    ...
    link.transition().delay(nodes.length * 20 + 100).duration(300).style('opacity', 1);
    ...
    node
      .on('mouseover', (event, d) => {
        tooltip
          ...
          .html(`<strong>${d.title}</strong><br/><span class="badge badge-muted" style="font-size:10px">${d.category}</span>`);
      })
      .on('mousemove', (event) => {
        tooltip
          .style('left', (event.offsetX + 12) + 'px')
